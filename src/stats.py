...
from preprocess import clean_df, categorize_micro, mice
...
def imitate_preprocess(df, filter=None):
    df = df.reset_index()
    df = df.dropna(subset='Micro')
    df = calculate_age(df)
    ...
    df['Micro'] = df['Micro'].apply(lambda x: categorize_micro(x))
...
if __name__ == '__main__':
    ...
    true_df, dfs, mask, true_means, true_stds, columns, (indices, values, operator_cols) = imitate_preprocess(df_imp)
    ...
    true_df_0, _, _, true_means_0, true_stds_0, columns, (_, _, _) = imitate_preprocess(df_imp, filter=0)
    true_df_1, _, _, true_means_1, true_stds_1, columns, (_, _, _) = imitate_preprocess(df_imp, filter=1)
    true_df, _, _, true_means, true_stds, columns, (_, _, _) = imitate_preprocess(df_imp)
    ...
    preds = []
    for i in range(10):
        df = pd.read_csv(f'../data/preprocessed_mice_test/test_{i}.csv')
        dfs.append(df)

    preds = [df.tolist() for df in preds]

    file_path = f'outputs/{model_name}/{model_name}_ensemble_preds.txt'
    ...
    for y_model1, y_model2, df in zip(model_preds, preds, dfs):
        y_true = df['Micro']
