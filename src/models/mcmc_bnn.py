...
def hypertrain_ensemble_mcmc_bnn(xs_train, ys_train, xs_val, ys_val, xs_test, ys_test, df_cols,
                                 shap_selected):
    models = []
    posterior_samples = []
    models_params = []
    ...
    for idx, (X_train, y_train, X_val, y_val) in enumerate(zip(xs_train, ys_train, xs_val, ys_val)):
        ...
        model_path = f"./models/mcmc_bnn/model_{idx}.pth"

        # saving the model
        torch.save(best_model.state_dict(), model_path)

        # saving the mcmc samples
        torch.save(posterior_sample, f'./models/mcmc_bnn/bnn_posterior_samples_{idx}.pth')

        # saving model params to txt file
        with open(f'./models/mcmc_bnn/model_params_{idx}.txt', 'w') as txt_f:
    ...
    plot_accuracies(ensemble_reports, 'Ensemble', name=f'mcmc_bnn')

    plot_combined_roc_curve(ys_test, ensemble_pred_probas, 'mcmc_bnn')  # Plot combined ROC

    plot_cm(ensemble_cms[0], name='mcmc_bnn')

    r, _ = test(y=ys_test[0], y_pred=ensemble_preds[0])
    print('Results on one data set: ', r)

    with open(f'outputs/mcmc_bnn/mcmc_bnn.csv', 'w') as f:
    ...
    with open(f'outputs/mcmc_bnn/mcmc_bnn_performance_metrics.txt', 'w') as f:
        f.write(performance_string)
        f.close()

    interpret_mcmc_bnn(xs_train[0], xs_test[0], df_cols)
...
def predict_mcmc_bnn_models(data):
    """
    ...
    checkpoint_files = [f for f in os.listdir("./models/mcmc_bnn/") if f.endswith('.pth') and 'model' in f]
    ...
    for checkpoint_file in checkpoint_files:
        ...
        with open(f"./models/mcmc_bnn/model_params_{model_index}.txt", "r") as file:
            # Read the first line
        ...
        posterior_samples = torch.load(f'models/mcmc_bnn/bnn_posterior_samples_{model_index}.pth')
...
def interpret_mcmc_bnn(x_train, x_test, df_cols):
    """
    ...
    Returns:
        None
    """
    explainer = shap.KernelExplainer(model=lambda x: predict_mcmc_bnn_models(x),
    ...
    shap.save_html(f'outputs/mcmc_bnn/force_plot.htm', f)
    plt.close()
    ...
    f = plt.gcf()
    f.savefig(f'outputs/mcmc_bnn/summary_bar.png', bbox_inches='tight', dpi=300)
    plt.close()
    ...
    f = plt.gcf()
    f.savefig(f'outputs/mcmc_bnn/beeswarm.png', bbox_inches='tight', dpi=300)
    plt.close()
