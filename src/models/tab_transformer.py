...
def hypertrain_ensemble_tab_transformer(xs_train, ys_train, xs_val, ys_val, xs_test, ys_test, df_cols,
                                        shap_selected, interpret_model=False, testing=False):
    models = []
    models_params = []
...
def evaluate_ensemble_tab_transformer(xs_test, ys_test, df_cols, shap_selected,
    ...
    for checkpoint_file in checkpoint_files:
        model = load_pytorch_model(checkpoint_file, model_name, df_cols=df_cols)
        models.append(model)
...
def finetune_ensemble_tab_transformer(xs_finetune, ys_finetune, xs_val, ys_val, xs_test, ys_test, xs_pro, ys_pro,
                                      df_cols,
                                      shap_selected, interpret_model=True, testing=False):
    """
    ...
    for idx, (model, X_finetune, y_finetune, X_val, y_val) in enumerate(
        ...
        model_path = f"{finetuned_model_dir}/model_{idx}.pth"
        torch.save(model.state_dict(), model_path)

        # Save model parameters
        with open(f'{finetuned_model_dir}/model_params_{idx}.txt', 'w') as txt_f:
    ...
    # Optionally evaluate models
    if testing:
        evaluate_ensemble_tab_transformer(xs_test, ys_test, xs_pro, ys_pro, df_cols, shap_selected,
                                          model_name=model_name + '_finetuned')
...
def hypertrain_tab_transformer_model(x_train, y_train, x_val, y_val, df_cols, scoring='neg_log_loss',
    ...
    for _ in range(n_iter):
        ...
        for train_index, val_index in kf.split(x):
            ...
            model = PLTabTransformer(**sampled_params, df_cols=df_cols)
