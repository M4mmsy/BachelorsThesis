...
def hypertrain_ensemble_gandalf(xs_train, ys_train, xs_val, ys_val, xs_test, ys_test, df_cols,
    ...
    if interpret_model:
        interpret(xs_train[0], xs_test[0], df_cols=df_cols)
...
def evaluate_ensemble_gandalf(xs_test, ys_test, df_cols, shap_selected,
                              model_name='gandalf'):
    models = []
...
def hypertrain_gandalf_model(x_train, y_train, x_val, y_val, df_cols, cv=5, n_iter=30):
    """
...
# def make_ensemble_preds_gandalf(x_test, y_test, df_cols, models):
#     """
...
#     checkpoint_files = [f for f in os.listdir(f"./models/gandalf/") if f.endswith('.pth') and 'model' in f]
#
...
#
#
# def interpret_gandalf_model(x_train, x_test, df_cols):
#     """
...
#         None
#     """
#     explainer = shap.KernelExplainer(model=lambda x: predict_gandalf_models(x), data=shap.sample(x_train, 50), feature_names=df_cols)
#
...
#         show=False)
#     shap.save_html(f'outputs/gandalf/force_plot.htm', f)
#     plt.close()
#
#     fig, ax = plt.subplots()
...
#
#     f = plt.gcf()
#     f.savefig(f'outputs/gandalf/summary_bar.png', bbox_inches='tight', dpi=300)
#     plt.close()
#
#     fig, ax = plt.subplots()
#
...
#
#     f = plt.gcf()
#     f.savefig(f'outputs/gandalf/beeswarm.png', bbox_inches='tight', dpi=300)
#     plt.close()
