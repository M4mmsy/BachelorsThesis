...
def hypertrain_nested_light_gbm(X, y, df_cols, shap_selected, n_repeats, scaling=False, imputing=False, corr_thresh=0.85, target_corr_thresh=0.1):
...
def hypertrain_ensemble_light_gbm(xs_train, ys_train, xs_val, ys_val, xs_test, ys_test, df_cols,
    ...
    for idx, (X_train, y_train, X_val, y_val, X_test, y_test) in enumerate(zip(xs_train, ys_train, xs_val, ys_val, xs_test, ys_test)):
        ...
        models.append(hypertrain_light_gbm_model(X_train, y_train, X_val, y_val, X_test, y_test))
...
def evaluate_ensemble_light_gbm(xs_test, ys_test, df_cols, shap_selected,
                                model_name='light_gbm'):
...
def finetune_ensemble_light_gbm(xs_finetune, ys_finetune, xs_val, ys_val, xs_test, ys_test, xs_pro, ys_pro, df_cols,
    ...
    finetuned_model_path = f'{finetuned_model_dir}/model.pickle'
    ...
    # Optionally interpret models
    if interpret_model:
        interpret(xs_finetune[0], xs_test[0], df_cols, 
                  model_name=model_name + '_finetuned')

    # Optionally evaluate models
    if testing:
        evaluate_ensemble_light_gbm(xs_test, ys_test, xs_pro, ys_pro, df_cols, shap_selected,
                                    model_name=model_name + '_finetuned')


# def interpret_light_gbm(x_train, x_test, df_cols,  model_name='light_gbm'):
#     """
...
#         None
#     """
#     explainer = shap.KernelExplainer(model=lambda x: predict_light_gbm_model(x, 
#                                                                              model_name=model_name),
...
#     shap.save_html(f'outputs/{model_name}/force_plot.htm', f)
#     plt.close()
#
#     fig, ax = plt.subplots()
...
#
#
# def predict_light_gbm_model(data,  model_name='light_gbm'):
#     """
