...
def hypertrain_ensemble_ffn(xs_train, ys_train, xs_val, ys_val, xs_test, ys_test, df_cols,
    ...
    for idx, (X_train, y_train, X_val, y_val, X_test, y_test) in enumerate(zip(xs_train, ys_train, xs_val, ys_val, xs_test, ys_test)):
        ...
        best_model, model_params = hypertrain_pytorch_model(X_train, y_train, X_val, y_val)
        models.append(best_model)
        models_params.append(model_params)

        # Save model state dict
        model_path = f"./models/{model_name}/model_{idx}.pth"
        torch.save(best_model.state_dict(), model_path)

        # Save model parameters
        with open(f'./models/{model_name}/model_params_{idx}.txt', 'w') as txt_f:
...
def evaluate_ensemble_ffn(xs_test, ys_test, df_cols, shap_selected,
                          model_name='ffn'):
...
def hypertrain_pytorch_model(x_train, y_train, x_val, y_val, cv=5, n_iter=10, max_epochs=100, early_stopping_rounds=30):
    ...
    for _ in range(n_iter):
        ...
        for train_index, val_index in kf.split(x):
            ...
            model = NeuralNetwork(**sampled_params)
...
#
#
# def predict_pytorch_models(data):
#     """
...
#     checkpoint_files = [f for f in os.listdir("./models/ffn/") if f.endswith('.pth')]
#
...
#         with open(f"./models/ffn/model_params_{model_index}.txt", "r") as file:
#             # Read the first line
...
#
#
# def interpret_pytorch_model(x_train, x_test, df_cols):
#     """
...
#         None
#     """
#     explainer = shap.KernelExplainer(model=lambda x: predict_pytorch_models(x), data=shap.sample(x_train, 50), feature_names=df_cols)
#
...
#         show=False)
#     shap.save_html(f'outputs/ffn/force_plot.htm', f)
#     plt.close()
#
#     fig, ax = plt.subplots()
...
#
#     f = plt.gcf()
#     f.savefig(f'outputs/ffn/summary_bar.png', bbox_inches='tight', dpi=300)
#     plt.close()
#
#     fig, ax = plt.subplots()
#
...
#
#     f = plt.gcf()
#     f.savefig(f'outputs/ffn/beeswarm.png', bbox_inches='tight', dpi=300)
#     plt.close()
